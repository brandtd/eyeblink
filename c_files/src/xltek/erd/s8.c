...
////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
xltek_erd_file_t *xltek_openErdFileS8( const char *filename )
{
  ...
  // Verify that this is a schema 8 ERD file.
  if (!xltek_uuidEqual( erd_file->gen_header.m_file_guid, UUID_ERD ) ||
      erd_file->gen_header.m_file_schema != 8) {
    fprintf( stderr, "Given file '%s' is not a schema 8 Raw Data file!\n",
             filename );
    free( erd_file );
    return NULL;
  }
  ...
  fread(&(erd_file->erd_head), sizeof(xltek_erd_s8_t), 1, file);
...
////////////////////////////////////////////////////////////////////////////////
////////////////////////////////////////////////////////////////////////////////
const char **xltek_getErdLabelsS8( xltek_erd_file_t *file )
{
  int i;
  char const **labels;
  ...
  labels = (char const**) malloc(sizeof(char)*file->erd_head.s8.m_num_channels);
  ...
  #define VALID_INDEX_IN( labels_array ) \
          ((file->erd_head.s8.m_phys_chan[i] < \
            sizeof((labels_array)) / sizeof(char*)) &&\
          (file->erd_head.s8.m_phys_chan[i] >= 0))

  #define CASE_STATEMENT(c,la) \
    case (c):\
      if (VALID_INDEX_IN((la))) {\
        labels[i] = (la)[file->erd_head.s8.m_phys_chan[i]];\
      } else {\
        fprintf( stderr, "Invalid physical channel index (%d) in '%s'!\n", \
                 file->erd_head.s8.m_phys_chan[i], file->filename ); \
        free(labels); \
        return NULL; \
      }\
      break

  for (i = 0; i < file->erd_head.s8.m_num_channels; i++) {
    switch (file->erd_head.s8.m_headbox_type[0]) {
      ...
      default:
        fprintf( stderr, "Given file\n"
                 ...
                 file->filename, file->erd_head.s8.m_headbox_type[0] );
        free(labels);
        return NULL;
    }
  }
