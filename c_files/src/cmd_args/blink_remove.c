...
{
  printf( "Usage:\n"
"  %s [options] -in IN_FILE -o OUT_FILE\n"
"\n"
"  DESCRIPTION:\n"
"\n"
"    Using the blink source detection code, this program removes blinks from\n"
"    EEG data given in the specified EDF file, IN_FILE, storing the results\n"
"    in the specified EDF file, OUT_FILE.\n"
"\n"
...
unsigned int parseArgs( int *argc, char ***argv, CmdLineArgs *cmd_args )
{
  ...
  for (i = 1; i < *argc;) {
    if (strncmp("-", (*argv)[i], 1) == 0) {
      ...
      if (PARAM_EQUALS( "-c", "--contrast" )) {
        ...
        i += 2;
      } else if (PARAM_EQUALS( "-in", "--in-file")) {
        cmd_args->in_file = (*argv)[i+1];

        if ((file = fopen( cmd_args->in_file, "r" )) == NULL) {
  ...
  if (cmd_args->in_file == NULL || cmd_args->out_file == NULL) {
    usage( (*argv)[0] );
    return 0;
  }
