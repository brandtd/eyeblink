...
%   Q = cumulant( X, M )
%
%   Generates a cumulant matrix, `Q', for the observations in `X' according to
%   the the matrix `M' and the equation for Q, defined entry-wise as:
%
%                        n
%     Q_X(M) <=> q_ij = sum( cum( X_i, X_j, X_k, X_l ) * m_kl ), 1 <= i,j <= n
%                      k,l = 1
%
%   where `n' is the number of variables in `X' and `cum( a,b,c,d )' is the
%   fourth-order cumulant of variables a,b,c,d, defined as
%
...
%   of zero-mean variables, where each row of X represents a variable, and each
